...
export async function getMedicationChartByResident(residentId) {
    const { data } = await api.get(`/medication-charts/resident/${residentId}`);
    return data;
}
...
export async function updateMedication(chartId, medicationId, payload) {
    const { data } = await api.put(`/medication-charts/${chartId}/medications/${medicationId}`, payload);
    return data;
}
